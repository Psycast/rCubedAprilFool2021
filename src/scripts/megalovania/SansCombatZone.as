package scripts.megalovania
{
    ...
    public class SansCombatZone extends Sprite implements ISansTickable
    {
        ...
        public function CombatZoneTick(eclipsed:Number):void
        {
            ...
            if (EndResize != null && Math.abs(BBox.left - TargetLeft) <= 2 && Math.abs(BBox.top - TargetTop) <= 2 && Math.abs(BBox.bottom - TargetBottom) <= 2 && Math.abs(BBox.right - TargetRight) <= 2)
            {
                Border_X = TargetLeft;
                Border_Y = TargetTop;
                Border_Height = TargetBottom - TargetTop;
        ...
        {
            // Bounds Check
            if (heart.x < Border_X + Border_Stroke + 8)
                return true;
            if (heart.y < Border_Y + Border_Stroke + 8)
                return true;
            if (heart.x > Border_X + Border_Width - Border_Stroke - 8)
                return true;
            if (heart.y > Border_Y + Border_Height - Border_Stroke - 8)
                return true;
