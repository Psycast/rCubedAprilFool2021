package scripts.megalovania
{
    ...
    public class SansVPAD
    {
        ...
        public function update(keyDowns:Array):void
        {
            ...
            if (keyDowns[Keyboard.A] || keyDowns[Keyboard.LEFT])
                this.L = 1;
            if (keyDowns[Keyboard.S] || keyDowns[Keyboard.DOWN])
                this.D = 1;
            if (keyDowns[Keyboard.W] || keyDowns[Keyboard.UP])
                this.U = 1;
            if (keyDowns[Keyboard.D] || keyDowns[Keyboard.RIGHT])
                this.R = 1;
            if (keyDowns[Keyboard.Z] || keyDowns[Keyboard.ENTER])
                this.Confirm = 1;
            if (keyDowns[Keyboard.X] || keyDowns[Keyboard.SHIFT])
                this.Cancel = 1;
            if (keyDowns[Keyboard.C] || keyDowns[Keyboard.CONTROL])
                this.Menu = 1;
