package scripts.megalovania
{
    ...
    public class SansBoneH extends Sprite implements ISansTickable, ISansDamageable
    {
        ...
        public function SansBoneH(container:SansAttackContainer, X:Number, Y:Number, Width:Number, Direction:int, Speed:Number, Color:int = 0)
        {
            ...
            if (!BITMAP_DATA)
                BITMAP_DATA = new SansBattleBoneH() as BitmapData;
        ...
        public function update(delta:Number, eclipsed:Number, script:SansBattleScript):void
        {
            ...
            if (Direction == 0 && this.x > SansAttackContainer.CONTAINER_WIDTH)
                script.RemoveTickable(this);
            if (Direction == 1 && this.y > SansAttackContainer.CONTAINER_HEIGHT)
                script.RemoveTickable(this);
            if (Direction == 2 && this.x < -gfx.width)
                script.RemoveTickable(this);
            if (Direction == 3 && this.y < -gfx.height)
                script.RemoveTickable(this);
        }
