package scripts
{
    ...
    import scripts.megalovania.ISansTickable;
    import scripts.megalovania.SansAttackContainer;
    import scripts.megalovania.SansRPGTextLite;
    import scripts.megalovania.SansSoundObject;
    import scripts.megalovania.SansVPAD;
    ...
    public class SansBattleScript implements ILevelScript
    {
        ...
        public function doFrameEvent(frame:int):void
        {
            ...
            if ((frame == lastFrame || skipLevel) && !itHasBegun) // 1745
            {
                ...
                gameplay.addEventListener(Event.ENTER_FRAME, e_onEnterFrame, false, int.MAX_VALUE - 9, true);
        ...
        public function doCaseEvent(event:int):void
        {
            switch (event)
            {
                ...
                case 0:
                    ...
                    tl.staggerTo(chr, 1.5, {"autoAlpha": 0}, 0.15);

                    // Fake Receptor
                    var referenceReceptor:* = gameplay.noteBox.receptorArray[2];
                    var point:Point = referenceReceptor.localToGlobal(new Point());

                    var fake_recp:* = _noteskins.getReceptor(gameplay.options.noteskin, "U");
                    fake_recp.x = point.x;
                    fake_recp.y = point.y;
                    fake_recp.rotation = referenceReceptor.rotation;
                    fake_recp.scaleX = fake_recp.scaleY = referenceReceptor.scaleX;
                    ...
                    tl.to(fake_recp, 3, {"rotation": 0,
                            "x": 320 + player_attack_container.x,
                            "y": 304 + player_attack_container.y,
                            "ease": SineInOut.ease,
                            "overwrite": "auto"}, "+=2");
                    tl.to(fake_recp, 2.5, {"scaleX": 0.35, "scaleY": 0.35, "ease": SineInOut.ease, "overwrite": "auto"}, "-=1.5");

                    var fade_delay:Number = 1.05;
                    for (i = 16; i > 0; i--)
                    {
                        tl.to(fake_recp, 0.01, {"alpha": (i & 1), "overwrite": "auto"}, "+=" + fade_delay.toString());
                        fade_delay /= 1.3;
                    }
                    tl.to(fake_recp, 0.01, {"alpha": 0, "overwrite": "auto"}, "+=" + fade_delay.toString());
        ...
        {
            gameplay.stage.addEventListener(KeyboardEvent.KEY_DOWN, e_onKeyPress, false, int.MAX_VALUE - 9, true);
            gameplay.stage.addEventListener(KeyboardEvent.KEY_UP, e_onKeyPress, false, int.MAX_VALUE - 9, true);
        }
    }
}
