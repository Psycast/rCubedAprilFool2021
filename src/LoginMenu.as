    ...
    public class LoginMenu extends MenuPanel
    {
        ...
        private function loginLoadComplete(e:Event):void
        {
            ...
            else if (_data.result >= 1 && _data.result <= 3)
            {
                ...
                _gvars.activeUser.addEventListener(GlobalVariables.LOAD_COMPLETE, e_userDataLoad);
                _gvars.activeUser.addEventListener(GlobalVariables.LOAD_ERROR, e_userDataFailure);
            }
            else
            {
        ...
        private function e_userDataLoad(e:Event):void
        {
            _gvars.activeUser.removeEventListener(GlobalVariables.LOAD_COMPLETE, e_userDataLoad);
            _gvars.activeUser.removeEventListener(GlobalVariables.LOAD_ERROR, e_userDataFailure);
            _gvars.playerUser.saveLocal();
            _gvars.gameMain.beginTransition();
            switchTo(Main.GAME_MENU_PANEL);
        }

        private function e_userDataFailure(e:Event):void
        {
            _gvars.activeUser.removeEventListener(GlobalVariables.LOAD_COMPLETE, e_userDataLoad);
            _gvars.activeUser.removeEventListener(GlobalVariables.LOAD_ERROR, e_userDataFailure);
            setFields(false, false);
            _gvars.gameMain.addAlert("Unable to load User Data");
            isLoading = false;
        }
