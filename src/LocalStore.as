package
{
    ...
    public class LocalStore
    {
        ...
        /**
         * Imports settings from the main SharedObject into the April Fools 2021 SharedObject.
         */
        public static function doDataImport():void
        {
            var didImport:Boolean = getVariable("did_af2021_import", false);
            if (!didImport)
            {
                var so:SharedObject = SharedObject.getLocal(Constant.MAIN_LOCAL_SO_NAME);

                for (var key:String in so.data)
                {
                    var element:Object = so.data[key];
                    if (element != null)
                    {
                        setVariable(key, element);
                    }
                }
                so.close();
                setVariable("did_af2021_import", true);
                setVariable("menuMusicSoundVolume", 0.15);
                flush();
            }
        }
    }
