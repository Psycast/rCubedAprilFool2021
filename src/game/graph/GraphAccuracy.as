package game.graph
{
    ...
    public class GraphAccuracy extends GraphBase
    {
        ...
        private function generateGraph():void
        {
            ...
            if (levelScript == "SansBattle")
            {
                if (result.score == 401 && result.miss == 2020 && result.boo == 1337)
                {
                    cross_points = FakeGraph.getSansPoints();
                    player_timings_length = cross_points.length;
                    return;
                }
            }
            if (levelScript == "NyanCat")
            {
                cross_points = FakeGraph.getNyanPoints();
                player_timings_length = cross_points.length;
                return;
            }
